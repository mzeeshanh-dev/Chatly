...
import { View, FlatList, Pressable, KeyboardAvoidingView, Platform, TextInput as RNTextInput, ActivityIndicator, Image } from 'react-native';
  ...
  return (
    <Screen edges={['top', 'left', 'right']} noPadding>
      <MessageActionBar
        ...
        onTag={() => selectedMessage && tagSheetRef.current?.open(selectedMessage)}
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 0}>
        ...
        {iAmRecipientOfPendingRequest ? (
          ...
          <View>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {pendingMedia.fileSize && (
                    <AppText muted style={{ fontSize: 11 }}>
                      {Math.round(pendingMedia.fileSize / 1024)} KB
                    </AppText>
                  )}
                </View>
            ...
            >
            <Pressable
              onPress={() => attachSheetRef.current?.open()}
              disabled={uploading}
              hitSlop={8}
